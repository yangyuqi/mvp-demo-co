...
public class AliyunDownloadManager {
     ...
     * 并行下载最大数量,默认3
     */
    private int mMaxNum = 13;
    /**
     * 下载路径
     */
    private String downloadDir = BasisConstants.Aliyun.ALICACHE + "/AliPlayerDownload";
    /**
     * 加密文件路径
     */
    private String encryptFilePath = BasisConstants.Aliyun.CACHE_PATH + "/aliyunPlayer/encryptedApp.dat";
     ...
     * 准备下载项目
     */
    public void prepareDownload(final VidAuth vidAuth, final long videoSize, final String userId, final String videoCover, final String sectionName, final Integer sectionId, final String videoId) {
...
//                        //一个JniDownloader 对应多个 AliyunDownloaderMediaInfo(同一Vid,不同清晰度)
                        ...
                        downloadMediaInfo.setSize(videoSize);
                        downloadMediaInfo.setStatus(AliyunDownloadMediaInfo.Status.Prepare);
                        downloadMediaInfo.setmVidAuth(vidAuth);
                        downloadMediaInfo.setUserId(userId);
                        downloadMediaInfo.setVideoCover(videoCover);
                        downloadMediaInfo.setSectionName(sectionName);
                        downloadMediaInfo.setSectionId(sectionId);
                        downloadMediaInfo.setVideoId(videoId);
...
// TODO: 2019/9/29  playauth下载
    ...
    public void prepareDownloadByQuality(final AliyunDownloadMediaInfo downloadMediaInfo, final int intentState) {
        if (downloadMediaInfo == null || downloadMediaInfo.getmVidAuth() == null) {
            return;
        }
        final List<AliyunDownloadMediaInfo> downloadMediaInfos = new ArrayList<>();
        final AliMediaDownloader jniDownloader = AliDownloaderFactory.create(mContext);
...
//                        //一个JniDownloader 对应多个 AliyunDownloaderMediaInfo(同一Vid,不同清晰度)
        ...
        jniDownloader.prepare(downloadMediaInfo.getmVidAuth());
    }
    ...
    public void startDownload(final AliyunDownloadMediaInfo downloadMediaInfo) {
        ...
        //如果没有auth，则是恢复数据的操作，需要重新请求auth

        if (downloadMediaInfo.getmVidAuth() == null) {
            getVideAuth(downloadMediaInfo, INTENT_STATE_START);
        } else {
            //直接开始下载
            //判断磁盘空间是否足够
            if (DownloadUtils.isStorageAlarm(mContext, downloadMediaInfo)) {
                ...
                if (downloadingList.size() <= mMaxNum) {
                    ...
                    jniDownloader.selectItem(trackInfo.getIndex());
                    setListener(downloadMediaInfo, jniDownloader);
                    jniDownloader.updateSource(downloadMediaInfo.getmVidAuth());
                    jniDownloader.start();
                } else {
                    //防止重复添加
    ...
    public void stopDownload(AliyunDownloadMediaInfo downloadMediaInfo) {
        ...
        for (AliyunDownloadMediaInfo downloadMediaInfo1 : downloadInfos.keySet()) {
            if (downloadMediaInfo1.getVid().equals(downloadMediaInfo.getVid()) && downloadMediaInfo1.getQuality().equals(downloadMediaInfo.getQuality())) {
                AliMediaDownloader jniDownloader = downloadInfos.get(downloadMediaInfo1);
                if (jniDownloader == null) {
                    return;
                }
                jniDownloader.stop();
...
//            jniDownloader.deleteFile();
    ...
    private void getVideAuth(final AliyunDownloadMediaInfo downloadMediaInfo, final int intentState) {
        VidAuthUtil.getVidAuth(downloadMediaInfo.getVid(), new VidAuthUtil.OnAuthResultListener() {
            ...
            public void onFail() {
                ThreadUtils.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(mContext.getApplicationContext(), mContext.getResources().getString(R.string.alivc_player_get_auth_failed), Toast.LENGTH_SHORT).show();
                        if (innerDownloadInfoListener != null) {
                            innerDownloadInfoListener.onError(downloadMediaInfo, ErrorCode.ERROR_UNKNOWN_ERROR, mContext.getResources().getString(R.string.alivc_player_get_auth_failed), null);
                        }
                    }
                });
            }
    ...
    /**
     * 数据库查询该课程下所有下载中或下载完毕等。。的视频
     *
     * @param sectionId 节I，如果节Id为空 则用章ID
     * @param listener
     */
    public void findVideoInCourseByDb(final int sectionId, final LoadDbDatasListener listener) {
        if (mDatabaseManager != null) {
            ThreadUtils.runOnSubThread(new Runnable() {
                @Override
                public void run() {
                    ...
                    final List<AliyunDownloadMediaInfo> downloadMediaInfos = mDatabaseManager.selectAllByCourseId(sectionId, userId);
                    ThreadUtils.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            if (listener != null) {
                                listener.onLoadSuccess(downloadMediaInfos);
                            }
                        }
                    });
                }
